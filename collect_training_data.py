...
def generate_dataset(img, id, img_id):
    ...
    cv2.imwrite("data/train_"+str(id)+"_"+str(img_id), img)
...
imagePaths= [os.path.join("images/5", f) for f in os.listdir("images/5")]
...
for (i, imagePath) in enumerate(imagePaths):
    ...
    image = detect(image, faceCascade, name)
    
#Extract images from using live webcam     
#while True:
    #if img_id % 10 == 0:
